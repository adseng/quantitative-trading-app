...
import type { BoxRangeReversalParams } from '@/views/cozePredict/types'
import { useBoxRangeBacktestRunner } from './hooks/useBoxRangeBacktestRunner'

const STRATEGY_NAME = 'box-range-reversal'
...
export default function BoxRangeReversal() {
  ...
  const [params, setParams] = useState<BoxRangeReversalParams>({
    impulseLookback: 30,
  ...
  const { running, error: backtestError, report, execute } = useBoxRangeBacktestRunner()
  ...
  const updateParams = <K extends keyof BoxRangeReversalParams>(key: K, value: BoxRangeReversalParams[K]) => {
    setParams((current) => ({ ...current, [key]: value }))
  }

  return (
    <div className="max-w-7xl mx-auto p-4 space-y-4">
      <h1 className="text-xl font-medium text-[#242f57]">箱体震荡反转策略</h1>
      ...
      <Card title="回测参数">
        <div className="flex flex-wrap items-center gap-3">
          <span className="text-sm text-gray-600">冲击窗口</span>
          <InputNumber min={5} max={120} value={params.impulseLookback} onChange={(value) => updateParams('impulseLookback', value == null ? 30 : Number(value))} />
          <span className="text-sm text-gray-600">横盘窗口</span>
          <InputNumber min={4} max={60} value={params.consolidationLookback} onChange={(value) => updateParams('consolidationLookback', value == null ? 12 : Number(value))} />
          ...
          <span className="text-sm text-gray-600">最小冲击幅度</span>
          <InputNumber min={0.005} max={0.2} step={0.001} value={params.minImpulsePercent} onChange={(value) => updateParams('minImpulsePercent', value == null ? 0.03 : Number(value))} />
          <span className="text-sm text-gray-600">冲击强度倍数</span>
          <InputNumber min={0.5} max={10} step={0.1} value={params.minImpulseATRRatio} onChange={(value) => updateParams('minImpulseATRRatio', value == null ? 2.5 : Number(value))} />
          <span className="text-sm text-gray-600">最小箱体宽度</span>
          <InputNumber min={0.0005} max={0.05} step={0.0005} value={params.minBoxWidthPercent} onChange={(value) => updateParams('minBoxWidthPercent', value == null ? 0.003 : Number(value))} />
          <span className="text-sm text-gray-600">最大箱体宽度</span>
          <InputNumber min={0.001} max={0.1} step={0.001} value={params.maxBoxWidthPercent} onChange={(value) => updateParams('maxBoxWidthPercent', value == null ? 0.015 : Number(value))} />
          <span className="text-sm text-gray-600">横盘量能比</span>
          <InputNumber min={0.1} max={2} step={0.05} value={params.consolidationVolumeRatio} onChange={(value) => updateParams('consolidationVolumeRatio', value == null ? 0.7 : Number(value))} />
          <span className="text-sm text-gray-600">横盘波动比</span>
          <InputNumber min={0.1} max={2} step={0.05} value={params.consolidationATRRatio} onChange={(value) => updateParams('consolidationATRRatio', value == null ? 0.7 : Number(value))} />
          <span className="text-sm text-gray-600">边界触碰次数</span>
          <InputNumber min={1} max={10} value={params.minBoundaryTouches} onChange={(value) => updateParams('minBoundaryTouches', value == null ? 2 : Number(value))} />
          <span className="text-sm text-gray-600">边界容差</span>
          <InputNumber min={0} max={0.02} step={0.0005} value={params.edgeTolerancePercent} onChange={(value) => updateParams('edgeTolerancePercent', value == null ? 0.002 : Number(value))} />
          <span className="text-sm text-gray-600">影线/实体</span>
          <InputNumber min={0.1} max={5} step={0.1} value={params.minRejectWickBodyRatio} onChange={(value) => updateParams('minRejectWickBodyRatio', value == null ? 1.5 : Number(value))} />
          <span className="text-sm text-gray-600">止损 ATR 倍数</span>
          <InputNumber min={0.1} max={5} step={0.1} value={params.stopATRMultiplier} onChange={(value) => updateParams('stopATRMultiplier', value == null ? 0.8 : Number(value))} />
          <span className="text-sm text-gray-600">冷却 K 线</span>
          <InputNumber min={0} max={20} value={params.cooldownBars} onChange={(value) => updateParams('cooldownBars', value == null ? 2 : Number(value))} />
          <span className="text-sm text-gray-600">止盈因子(0-1)</span>
          <InputNumber min={0} max={1} step={0.05} value={params.takeProfitFactor} onChange={(value) => updateParams('takeProfitFactor', value == null ? 0.5 : Number(value))} />
          ...
          <InputNumber min={10} max={500} value={visibleBars} onChange={(value) => setVisibleBars(normalizeVisibleBars(value))} style={{ width: 90 }} />
