import { StartBatchTest, StopBatchTest, GetBatchTestProgress } from '../../../wailsjs/go/main/App'
...
interface TestCaseInfo {
  id: number
  name: string
  useMA: boolean; maShort: number; maLong: number; maWeight: number
  useTrend: boolean; trendN: number; trendWeight: number
  useRSI: boolean; rsiPeriod: number; rsiOverbought: number; rsiOversold: number; rsiWeight: number
  useMACD: boolean; macdFast: number; macdSlow: number; macdSignal: number; macdWeight: number
  useBoll: boolean; bollPeriod: number; bollMultiplier: number; bollWeight: number
  useBreakout: boolean; breakoutPeriod: number; breakoutWeight: number
  usePriceVsMA: boolean; priceVsMAPeriod: number; priceVsMAWeight: number
  useATR: boolean; atrPeriod: number; atrWeight: number
  useVolume: boolean; volumePeriod: number; volumeWeight: number
  useSession: boolean; sessionWeight: number
}
...
const columns: ColumnsType<TestResultRow> = [
  ...
  {
    title: '信号正确率', width: 100, align: 'right',
    render: (_, r) => {
  ...
  {
    title: '平均净分', width: 85, align: 'right',
    render: (_, r) => {
      const v = r.avgScore
  ...
  {
    title: '信号强度', width: 80, align: 'right',
    render: (_, r) => {
...
export default function BatchTest() {
  ...
  useEffect(() => {
    GetBatchTestProgress().then((info: any) => {
      setProgress(info.nextIndex ?? 0)
      setTotal(info.totalCases ?? 200)
      if (info.running) {
        setRunning(true)
      } else if (info.nextIndex > 0 && info.nextIndex < info.totalCases) {
        setPhase('stopped')
        setMessage(`上次已完成 ${info.nextIndex}/${info.totalCases}，点击继续`)
      } else if (info.nextIndex >= info.totalCases) {
        setPhase('done')
        setMessage('全部 200 组已完成')
      }
    })
  ...
  const handleStart = async () => {
    ...
    setRunning(true)
    setPhase('fetching')
    setMessage(isResume ? '继续中...' : '启动中...')
    if (!isResume) setRecent([])
    try {
      await StartBatchTest()
    } catch (e: any) {
      setMessage('启动失败: ' + e.toString())
      setRunning(false)
    }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <Text type="secondary" style={{ fontSize: 12 }}>
        V3方案：均值回归市场 · MA/Trend/MACD/PMA用负权重 · RSI/Boll用正权重 · Break/ATR/Vol/Sess用负权重 · 信号正确率{'>'}55% 标绿
      </Text>
    </div>
  )
}
